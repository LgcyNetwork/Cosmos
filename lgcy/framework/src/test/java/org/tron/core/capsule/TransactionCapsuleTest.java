...
public class TransactionCapsuleTest {
  ...
  public void addSign() {
    ...
    Transaction.Builder usdlBuilder = Transaction.newBuilder();
    ...
    usdlBuilder.setRawData(rawBuilder);
    AccountStore accountStore = dbManager.getAccountStore();
    TransactionCapsule transactionCapsule = new TransactionCapsule(usdlBuilder.build());
    //Accout not exist
    try {
  ...
  public void validateSignature0() {
    ...
    //SignatureFormatException
    ByteString test = ByteString.copyFromUtf8("test");
    usdlBuilder.clearSignature().addSignature(test);
    try {
      TransactionCapsule.validateSignature(usdlBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertFalse(true);
    } catch (PermissionException e) {
    ...
    new Random().nextBytes(rand);
    rand[64] = 8;  // v = 8 < 27 v += 35 > 35
    usdlBuilder.clearSignature().addSignature(ByteString.copyFrom(rand));
    try {
      TransactionCapsule.validateSignature(usdlBuilder.build(), hash,
          accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertEquals(e.getMessage(), "Header byte out of range: 35");
    ...
    usdlBuilder.clearSignature().addAllSignature(sign21_11);
    try {
      TransactionCapsule.validateSignature(usdlBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
    ...
    usdlBuilder.clearSignature().addAllSignature(sign21_11_22_23);
    try {
      TransactionCapsule
          .validateSignature(usdlBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertFalse(true);
    } catch (PermissionException e) {
    ...
    usdlBuilder.clearSignature().addAllSignature(sign21_22_21);
    try {
      TransactionCapsule
          .validateSignature(usdlBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    ...
    usdlBuilder.clearSignature().addAllSignature(sign21);
    try {
    ...
    usdlBuilder.clearSignature().addAllSignature(sign21_22);
    try {
      boolean result = TransactionCapsule
          .validateSignature(usdlBuilder.build(), hash, accountStore);
    ...
    usdlBuilder.clearSignature().addAllSignature(sign21_22_23);
    try {
      boolean result = TransactionCapsule
          .validateSignature(usdlBuilder.build(), hash, accountStore);
      Assert.assertTrue(result);
  ...
  public void validateSignature1() {
    ...
    ArrayList<ByteString> sign = sign(prikeys, Sha256Hash.hash(rawBuilder.build().toByteArray()));
    usdlBuilder.addAllSignature(sign);
    transactionCapsule = new TransactionCapsule(usdlBuilder.build());

    // no contract
    prikeys.clear();
    prikeys.add(ByteArray.fromHexString(KEY_21));
    usdlBuilder = Transaction.newBuilder();
    ...
    usdlBuilder = transactionCapsule.getInstance().toBuilder();
    //SignatureFormatException
    ByteString test = ByteString.copyFromUtf8("test");
    usdlBuilder.clearSignature();
    usdlBuilder.addSignature(test);
    transactionCapsule = new TransactionCapsule(usdlBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    rand[64] = 8;  // v = 8 < 27 v += 35 > 35
    usdlBuilder.clearSignature();
    usdlBuilder.addSignature(ByteString.copyFrom(rand));
    transactionCapsule = new TransactionCapsule(usdlBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_11 = sign(prikeys, hash);
    usdlBuilder.clearSignature();
    usdlBuilder.addAllSignature(sign21_11);
    transactionCapsule = new TransactionCapsule(usdlBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_11_22_23 = sign(prikeys, hash);
    usdlBuilder.clearSignature();
    usdlBuilder.addAllSignature(sign21_11_22_23);
    transactionCapsule = new TransactionCapsule(usdlBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_22_21 = sign(prikeys, hash);
    usdlBuilder.clearSignature();
    usdlBuilder.addAllSignature(sign21_22_21);
    transactionCapsule = new TransactionCapsule(usdlBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21 = sign(prikeys, hash);
    usdlBuilder.clearSignature();
    usdlBuilder.addAllSignature(sign21);
    transactionCapsule = new TransactionCapsule(usdlBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_22 = sign(prikeys, hash);
    usdlBuilder.clearSignature();
    usdlBuilder.addAllSignature(sign21_22);
    transactionCapsule = new TransactionCapsule(usdlBuilder.build());
    try {
    ...
    ArrayList<ByteString> sign21_22_23 = sign(prikeys, hash);
    usdlBuilder.clearSignature();
    usdlBuilder.addAllSignature(sign21_22_23);
    transactionCapsule = new TransactionCapsule(usdlBuilder.build());
    try {
      boolean result = transactionCapsule.validateSignature(dbManager);
      Assert.assertTrue(result);
  ...
  @Test
  public void usdlCapsuleClearTest() {
    Transaction tx = Transaction.newBuilder()
        .addRet(Result.newBuilder().setContractRet(contractResult.OUT_OF_TIME).build()).build();
    TransactionCapsule usdlCap = new TransactionCapsule(tx);
    Result.contractResult contractResult = usdlCap.getContractResult();
    usdlCap.resetResult();
    Assert.assertEquals(usdlCap.getInstance().getRetCount(), 0);
    usdlCap.setResultCode(contractResult);
    Assert.assertEquals(usdlCap.getInstance()

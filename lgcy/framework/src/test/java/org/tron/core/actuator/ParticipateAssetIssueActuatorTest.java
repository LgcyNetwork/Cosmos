...
public class ParticipateAssetIssueActuatorTest {
  ...
  public void sameTokenNameCloseRightAssetIssue() {
    ...
    try {
      ...
      Assert.assertEquals(toAccount.getAssetMap().get(ASSET_NAME).longValue(),
          TOTAL_SUPPLY - (1000L) / USDL_NUM * NUM);
      //V2
  ...
  public void sameTokenNameCloseExchangeDevisibleTest() {
    ...
    try {
      ...
      Assert.assertEquals(
          ...
          TOTAL_SUPPLY - (999L * NUM) / USDL_NUM);
    } catch (ContractValidateException e) {
      Assert.assertFalse(e instanceof ContractValidateException);
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
  ...
  /**
   * SameTokenName close, not enough usdl
   */
  @Test
  public void sameTokenNameCloseNotEnoughUSDLTest() {
    initAssetIssue(chainBaseManager.getDynamicPropertiesStore()
            .getLatestBlockHeaderTimestamp() - 1000,
        chainBaseManager.getDynamicPropertiesStore().getLatestBlockHeaderTimestamp() + 1000);
    // First, reduce the owner usdl balance. Else can't complete this test case.
    AccountCapsule owner = chainBaseManager.getAccountStore()
        .get(ByteArray.fromHexString(OWNER_ADDRESS));
  ...
  /**
   * SameTokenName open, not enough usdl
   */
  @Test
  public void sameTokenNameOpenNotEnoughUSDLTest() {
    chainBaseManager.getDynamicPropertiesStore().saveAllowSameTokenName(1);
    initAssetIssue(chainBaseManager.getDynamicPropertiesStore()
            .getLatestBlockHeaderTimestamp() - 1000,
        chainBaseManager.getDynamicPropertiesStore().getLatestBlockHeaderTimestamp() + 1000);
  ...
  public void sameTokenNameCloseMultiplyOverflowTest() {
    initAssetIssue(chainBaseManager.getDynamicPropertiesStore()
            .getLatestBlockHeaderTimestamp() - 1000,
        chainBaseManager.getDynamicPropertiesStore().getLatestBlockHeaderTimestamp() + 1000);
    // First, increase the owner usdl balance. Else can't complete this test case.
    AccountCapsule owner = chainBaseManager.getAccountStore()
        .get(ByteArray.fromHexString(OWNER_ADDRESS));
  ...
  /**
   * SameTokenName close, exchangeAmount <= 0 usdl, throw exception
   */
  @Test
  ...
  /**
   * SameTokenName open, exchangeAmount <= 0 usdl, throw exception
   */
  @Test

...
public class AdvServiceTest {
  ...
  private void testAddInv() {
    boolean flag;
    Item itemUSDL = new Item(Sha256Hash.ZERO_HASH, InventoryType.USDL);
    flag = service.addInv(itemUSDL);
  ...
  private void testUSDLBroadcast() {
    Protocol.Transaction usdl = Protocol.Transaction.newBuilder().build();
    CommonParameter.getInstance().setValidContractProtoThreadNum(1);
    TransactionMessage msg = new TransactionMessage(usdl);
    service.broadcast(msg);
    Item item = new Item(msg.getMessageId(), InventoryType.USDL);
    Assert.assertNotNull(service.getMessage(item));
  }

...
import org.tron.protos.contract.SmartContractOuterClass.UpdateKandyLimitContract;


@Slf4j
//@Ignore
public class UpdateKandyLimitContractActuatorTest {

  private static final String dbPath = "output_updateKandyLimitContractActuator_test";
  ...
  private static final long SOURCE_KANDY_LIMIT = 10L;
  private static final long TARGET_KANDY_LIMIT = 30L;
  private static final long INVALID_KANDY_LIMIT = -200L;
  ...
  public static void init() {
    ...
    dbManager.getDynamicPropertiesStore()
        .statsByVersion(ForkBlockVersionConsts.KANDY_LIMIT, stats);
    VMConfig.inilvmHardFork(true);
  }
  ...
  public static void destroy() {
    ...
    CommonParameter.setKANDY_LIMIT_HARD_FORK(false);
  }
  ...
  public void createCapsule() {
    ...
    builder.setOriginKandyLimit(SOURCE_KANDY_LIMIT);
  ...
  private Any getContract(String accountAddress, String contractAddress, long originKandyLimit) {
    return Any.pack(
        UpdateKandyLimitContract.newBuilder()
            ...
            .setOriginKandyLimit(originKandyLimit).build());
  }

  @Test
  public void successUpdateKandyLimitContract() throws InvalidProtocolBufferException {
    UpdateKandyLimitContractActuator actuator = new UpdateKandyLimitContractActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager())
        .setAny(getContract(OWNER_ADDRESS, CONTRACT_ADDRESS, TARGET_KANDY_LIMIT));
    ...
    try {
      ...
      // assert result state and kandy_limit
      ...
      Assert.assertEquals(
          dbManager.getContractStore().get(ByteArray.fromHexString(CONTRACT_ADDRESS))
              .getOriginKandyLimit(), TARGET_KANDY_LIMIT);
  ...
  public void invalidAddress() {
    UpdateKandyLimitContractActuator actuator = new UpdateKandyLimitContractActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager())
        .setAny(getContract(OWNER_ADDRESS_INVALID, CONTRACT_ADDRESS, TARGET_KANDY_LIMIT));
  ...
  public void noExistAccount() {
    UpdateKandyLimitContractActuator actuator = new UpdateKandyLimitContractActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager())
        .setAny(getContract(OWNER_ADDRESS_NOTEXIST, CONTRACT_ADDRESS, TARGET_KANDY_LIMIT));
  ...
  @Test
  public void invalidResourceKandyLimit() {
    UpdateKandyLimitContractActuator actuator = new UpdateKandyLimitContractActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager())
        .setAny(getContract(OWNER_ADDRESS, CONTRACT_ADDRESS, INVALID_KANDY_LIMIT));
    ...
    try {
      ...
      fail("origin kandy limit less than 0");
    } catch (TronException e) {
      Assert.assertTrue(e instanceof ContractValidateException);
      Assert.assertEquals("origin kandy limit must be > 0", e.getMessage());
    }
  }
  ...
  public void noExistContract() {
    UpdateKandyLimitContractActuator actuator = new UpdateKandyLimitContractActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager())
        .setAny(getContract(OWNER_ADDRESS, NO_EXIST_CONTRACT_ADDRESS, TARGET_KANDY_LIMIT));
  ...
  public void callerNotContractOwner() {
    UpdateKandyLimitContractActuator actuator = new UpdateKandyLimitContractActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager())
        .setAny(getContract(SECOND_ACCOUNT_ADDRESS, CONTRACT_ADDRESS, TARGET_KANDY_LIMIT));
  ...
  @Test
  public void twiceUpdateKandyLimitContract() throws InvalidProtocolBufferException {
    UpdateKandyLimitContractActuator actuator = new UpdateKandyLimitContractActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager())
        .setAny(getContract(OWNER_ADDRESS, CONTRACT_ADDRESS, TARGET_KANDY_LIMIT));

    UpdateKandyLimitContractActuator secondActuator = new UpdateKandyLimitContractActuator();
  ...
  public void invalidContractType() {
    UpdateKandyLimitContractActuator actuator = new UpdateKandyLimitContractActuator();
    // create AssetIssueContract, not a valid UpdateKandyLimitContract contract , which will
    // throw e exception
    ...
    processAndCheckInvalid(actuator, ret, "contract type error",
        "contract type error, expected type [UpdateKandyLimitContract],real type["
  ...
  private void processAndCheckInvalid(UpdateKandyLimitContractActuator actuator,

...
public class TransactionTraceTest {
  ...
  public static void init() {
    ...
    dbManager.getDynamicPropertiesStore().saveTotalKandyWeight(100_000L);
    dbManager.getDynamicPropertiesStore().saveLatestBlockHeaderTimestamp(0);
    VMConfig.inilvmHardFork(false);
  ...
  public void testUseUsage()
    ...
    accountCapsule.setFrozenForKandy(5_000_000_000L, 0L);
    ...
    trace.exec();
    trace.pay();
    Assert.assertEquals(2050831L, trace.getReceipt().getKandyUsage());
    Assert.assertEquals(0L, trace.getReceipt().getKandyFee());
    Assert.assertEquals(205083100L,
        trace.getReceipt().getKandyUsage() * 100 + trace.getReceipt().getKandyFee());
    accountCapsule = dbManager.getAccountStore().get(accountCapsule.getAddress().toByteArray());
    Assert.assertEquals(totalBalance,
        accountCapsule.getBalance() + trace.getReceipt().getKandyFee());
  ...
  public void testTriggerUseFee()
    ...
    AccountCapsule originCapsule = new AccountCapsule(ByteString.copyFrom("origin".getBytes()),
        ByteString.copyFrom(Commons.decodeFromBase58Check(OwnerAddress)), AccountType.Normal,
        totalBalance);
    ownerCapsule.setFrozenForKandy(5_000_000_000L, 0L);
    originCapsule.setFrozenForKandy(5_000_000_000L, 0L);
    dbManager.getAccountStore()
    ...
    trace.exec();
    trace.pay();
    Assert.assertEquals(20252, trace.getReceipt().getKandyUsage());
    Assert.assertEquals(0, trace.getReceipt().getKandyFee());
    ownerCapsule = dbManager.getAccountStore().get(ownerCapsule.getAddress().toByteArray());
    Assert.assertEquals(totalBalance,
        trace.getReceipt().getKandyFee() + ownerCapsule
            .getBalance());
  }
  ...
  public void testTriggerUseUsage()
    ...
    accountCapsule.setFrozenForKandy(10_000_000L, 0L);
    dbManager.getAccountStore()
  ...
  private byte[] deployInit(Transaction transaction)
    ...
    trace.exec();
    trace.pay();
    Assert.assertEquals(0, trace.getReceipt().getKandyUsage());
    Assert.assertEquals(205083100L, trace.getReceipt().getKandyFee());
    accountCapsule = dbManager.getAccountStore().get(accountCapsule.getAddress().toByteArray());
    Assert.assertEquals(totalBalance,
        trace.getReceipt().getKandyFee() + accountCapsule
            .getBalance());
    ...
    Account account = Account.newBuilder()
        ...
        .setAccountResource(
            AccountResource.newBuilder()
                .setKandyUsage(1111111L)
                .setFrozenBalanceForPower(
                    Frozen.newBuilder()

...
public class TransactionStoreTest {
  ...
  public void getTransactionTest() throws BadItemException, ItemNotFoundException {
    ...
    final TransactionStore usdlStore = chainBaseManager.getTransactionStore();
    ...
    usdl.setBlockNum(blockCapsule.getNum());
    blockStore.put(blockCapsule.getBlockId().getBytes(), blockCapsule);
    usdlStore.put(usdl.getTransactionId().getBytes(), usdl);
    Assert.assertEquals("Get transaction is error",
        usdlStore.get(usdl.getTransactionId().getBytes()).getInstance(), usdl.getInstance());
    ...
    tc =
        TransferContract.newBuilder()
            ...
            .setToAddress(ByteString.copyFromUtf8("bbb"))
            .build();
    usdl = new TransactionCapsule(tc, ContractType.TransferContract);
    Assert.assertNull(usdlStore.get(usdl.getTransactionId().getBytes()));
  ...
  public void getUncheckedTransactionTest() {
    ...
    usdlStore.put(usdl.getTransactionId().getBytes(), usdl);
    Assert.assertEquals("Get transaction is error",
        usdlStore.getUnchecked(usdl.getTransactionId().getBytes()).getInstance(), usdl.getInstance());
    ...
    tc =
        TransferContract.newBuilder()
            ...
            .build();
    usdl = new TransactionCapsule(tc, ContractType.TransferContract);
    Assert.assertNull(usdlStore.getUnchecked(usdl.getTransactionId().getBytes()));

...
public class FreezeBalanceActuatorTest {
  ...
  private Any getContractForCpu(String ownerAddress, long frozenBalance, long duration) {
    return Any.pack(
        FreezeBalanceContract.newBuilder()
            ...
            .setResource(ResourceCode.USDL_POWER)
            .build());
  }
  ...
  @Test
  public void testFreezeBalanceForKandy() {
    ...
    try {
      ...
      Assert.assertEquals(frozenBalance, owner.getKandyFrozenBalance());
  ...
  public void testFreezeDelegatedBalanceForCpuSameNameTokenActive() {
    ...
    try {
      ...
      Assert.assertEquals(owner.getBalance(), initBalance - frozenBalance
          - TRANSFER_FEE);
      Assert.assertEquals(0L, owner.getFrozenBalance());
      Assert.assertEquals(frozenBalance, owner.getDelegatedFrozenBalanceForKandy());
      Assert.assertEquals(frozenBalance, owner.getTronPower());

      AccountCapsule receiver =
          dbManager.getAccountStore().get(ByteArray.fromHexString(RECEIVER_ADDRESS));
      Assert.assertEquals(frozenBalance, receiver.getAcquiredDelegatedFrozenBalanceForKandy());
      ...
      Assert.assertEquals(frozenBalance, delegatedResourceCapsule.getFrozenBalanceForPower());

      long totalKandyWeightAfter = dbManager.getDynamicPropertiesStore().getTotalKandyWeight();
      Assert.assertEquals(totalKandyWeightBefore + frozenBalance / 1000_000L,
          totalKandyWeightAfter);
  ...
  public void testFreezeDelegatedBalanceForCpuSameNameTokenClose() {
    ...
    try {
      ...
      Assert.assertEquals(owner.getBalance(), initBalance - frozenBalance
          - TRANSFER_FEE);
      Assert.assertEquals(0L, owner.getFrozenBalance());
      Assert.assertEquals(0L, owner.getDelegatedFrozenBalanceForKandy());
      Assert.assertEquals(0L, owner.getDelegatedFrozenBalanceForKandy());

      AccountCapsule receiver =
          dbManager.getAccountStore().get(ByteArray.fromHexString(RECEIVER_ADDRESS));
      Assert.assertEquals(0L, receiver.getAcquiredDelegatedFrozenBalanceForKandy());
      Assert.assertEquals(0L, receiver.getTronPower());

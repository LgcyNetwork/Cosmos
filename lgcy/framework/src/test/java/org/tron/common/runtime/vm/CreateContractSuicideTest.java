...
contract D {
  ...
  public void testAAfterAllowMultiSignProposal()
    ...
    this.manager.getDynamicPropertiesStore().saveAllowLvmTransferTrc10(1);
    ...
    Runtime aRuntime = LvmTestUtils
        .processTransactionAndReturnRuntime(aUSDL, DepositImpl.createRoot(manager), null);
    ...
    Transaction cUSDL = LvmTestUtils.generateDeploySmartContractAndGetTransaction(
        "testC", address, abi, cCode, value, fee, consumeUserResourcePercent, null, engeryLimit);
    Runtime cRuntime = LvmTestUtils
        .processTransactionAndReturnRuntime(cUSDL, DepositImpl.createRoot(manager), null);
    Assert.assertTrue(cRuntime.getResult().getException() instanceof OutOfKandyException);

    Transaction dUSDL = LvmTestUtils.generateDeploySmartContractAndGetTransaction(
        "testC", address, abi, dCode, value, fee, consumeUserResourcePercent, null, engeryLimit);
    Runtime dRuntime = LvmTestUtils
        .processTransactionAndReturnRuntime(dUSDL, DepositImpl.createRoot(manager), null);

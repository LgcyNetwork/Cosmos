...
public class Trc10Test extends VMContractTestBase {
  ...
  public void testTrc10Validate() throws ContractExeException,
    ...
    VMConfig.initAllowLvmAssetIssue(1);
    ...
    // deploy contract
    Protocol.Transaction usdl = LvmTestUtils.generateDeploySmartContractAndGetTransaction(
    ...
    runtime = LvmTestUtils.processTransactionAndReturnRuntime(usdl, manager, null);
    ...
    String hexInput = AbiUtil.parseMethod(methodTokenIssue, Arrays.asList(tokenP1, tokenP2,
        tokenP3, tokenP4));
    LVMTestResult result =
        LvmTestUtils.triggerContractAndReturnLvmTestResult(Hex.decode(OWNER_ADDRESS),
            factoryAddress,
    ...
    // validate assetissue assetname can't be usdl
    tokenP1 = "747278";
    tokenP2 = tokenP1;
    tokenP3 = 1000;
    tokenP4 = 2;
    ...
    hexInput = AbiUtil.parseMethod(methodTokenIssue, Arrays.asList(tokenP1, tokenP2, tokenP3,
        tokenP4));
    result = LvmTestUtils.triggerContractAndReturnLvmTestResult(Hex.decode(OWNER_ADDRESS),
        factoryAddress,
        Hex.decode(hexInput), value, fee, manager, null);
    Assert.assertEquals(ByteArray.toInt(result.getRuntime().getResult().getHReturn()), 0);
    Assert.assertNull(result.getRuntime().getRuntimeError());
    ...
    long expectKandyUsageTotal = 25000;
    long expectKandyUsageTotalMax = 30000;
    long reallyKandyUsageTotal = result.getReceipt().getKandyUsageTotal();
    // validate kandy cost
    Assert.assertTrue(reallyKandyUsageTotal > expectKandyUsageTotal
        && reallyKandyUsageTotal < expectKandyUsageTotalMax);
    ...
    // validate kandy cost
    expectKandyUsageTotal = 5000;
    expectKandyUsageTotalMax = 10000;
    reallyKandyUsageTotal = result.getReceipt().getKandyUsageTotal();

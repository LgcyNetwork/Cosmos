...
public class DepositTest {
  ...
  public void loopCallTest()
    ...
    Transaction aUSDL = LvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractA, address, aABI, aCode, value, fee, consumeUserResourcePercent,
        null, engeryLiimt);
    Runtime runtime = LvmTestUtils
        .processTransactionAndReturnRuntime(aUSDL, DepositImpl.createRoot(manager),
            null);
    Assert.assertNull(runtime.getRuntimeError());

    Transaction bUSDL = LvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractB, address, bABI, bCode, value, fee, consumeUserResourcePercent,
        null, engeryLiimt);
    runtime = LvmTestUtils
        .processTransactionAndReturnRuntime(bUSDL, DepositImpl.createRoot(manager), null);
    Assert.assertNull(runtime.getRuntimeError());

    byte[] aAddress = WalletUtil.generateContractAddress(aUSDL);
    byte[] bAddress = WalletUtil.generateContractAddress(bUSDL);
    ...
    // check result
    // expected: n1 = 1; n2 = 0
    byte[] checkN1Data = LvmTestUtils.parseAbi("n1()", null);
    byte[] checkN2Data = LvmTestUtils.parseAbi("n2()", null);

    LVMTestResult checkN1 = LvmTestUtils
        .triggerContractAndReturnLvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, checkN1Data, 0, fee, manager, null);
    LVMTestResult checkN2 = LvmTestUtils
        .triggerContractAndReturnLvmTestResult(Hex.decode(OWNER_ADDRESS),
    ...
    triggerData = LvmTestUtils.parseAbi("callBcallA(address,uint256,uint256)", params2);
    result = LvmTestUtils
        .triggerContractAndReturnLvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, triggerData, 0, fee, manager, null);
    Assert.assertNull(result.getRuntime().getRuntimeError());
    checkN1 = LvmTestUtils
        .triggerContractAndReturnLvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, checkN1Data, 0, fee, manager, null);
    checkN2 = LvmTestUtils
        .triggerContractAndReturnLvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, checkN2Data, 0, fee, manager, null);
    System.out.println(Hex.toHexString(checkN1.getRuntime().getResult().getHReturn()));
    System.out.println(Hex.toHexString(checkN2.getRuntime().getResult().getHReturn()));
  ...
  public void loopCallTestOldVersion()
    ...
    Runtime runtime = LvmTestUtils.processTransactionAndReturnRuntime(aUSDL, rootDeposit,
        null);
    Assert.assertNull(runtime.getRuntimeError());
    ...
    Transaction bUSDL = LvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    rootDeposit = DepositImpl.createRoot(manager);
    runtime = LvmTestUtils.processTransactionAndReturnRuntime(bUSDL, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());

...
public class TransferFailedKandyTest extends VMTestBase {
  /*
  ...
  contract KandyOfTransferFailedTest {
      ...
      // InsufficientBalance
      function testTransferUSDLInsufficientBalance() payable public{
          msg.sender.transfer(10);
      }

      function testSendUSDLInsufficientBalance() payable public{
          msg.sender.send(10);
      }
      ...
      function testCallUSDLInsufficientBalance(address payable caller) public {
          caller.call.value(10)(abi.encodeWithSignature("test()"));
      }

      function testCreateUSDLInsufficientBalance() payable public {
          (new Caller).value(10)();
      }

      // NonexistentTarget
      function testTransferUSDLNonexistentTarget(address payable nonexistentTarget) payable public {
      ...
      function testCallUSDLNonexistentTarget(address payable nonexistentTarget) payable public {
          require(address(this).balance >= 10);
      ...
      // target is self
      function testTransferUSDLSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.transfer(10);
      }

      function testSendUSDLSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.send(10);
      }
  ...
  contract KandyOfTransferFailedTest {
      ...
      function testCallUSDLInsufficientBalance(address caller) payable public {
          caller.call.value(10)(abi.encodeWithSignature("test()"));
      }
      ...
      // NonexistentTarget
      function testTransferUSDLNonexistentTarget(address nonexistentTarget) payable public {
          require(address(this).balance >= 10);
          nonexistentTarget.transfer(10);
      }
      ...
      function testCallUSDLNonexistentTarget(address nonexistentTarget) public {
          require(address(this).balance >= 10);
          nonexistentTarget.call.value(10)(abi.encodeWithSignature("test()"));
      }
  ...
  */
  ...
  TestCase[] testCasesAfterAllowLvmConstantinop = {
      new TestCase("testTransferUSDLSelf()", Collections.emptyList(), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testSendUSDLSelf()", Collections.emptyList(), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testSuicideNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testTransferUSDLNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testCallUSDLNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
  };
  TestCase[] testCasesBeforeAllowLvmConstantinop = {
      new TestCase("testTransferUSDLSelf()", Collections.emptyList(),
          true, contractResult.UNKNOWN),
      new TestCase("testSendUSDLSelf()", Collections.emptyList(),
          true, contractResult.UNKNOWN),
      new TestCase("testSuicideNonexistentTarget(address)",
  ...
  TestCase[] testCasesInsufficientBalance = {
      new TestCase("testTransferUSDLInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
      new TestCase("testSendUSDLInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.SUCCESS),
      new TestCase("testCreateUSDLInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
      new TestCase("testCallUSDLInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
  ...
  @Test
  public void testTransferFailedAfterAllowLvmConstantinopl()
      throws ContractExeException, ReceiptCheckErrException,
      VMIllegalException, ContractValidateException {
    ConfigLoader.disable = false;
    manager.getDynamicPropertiesStore().saveAllowLvmTransferTrc10(1);
    manager.getDynamicPropertiesStore().saveAllowLvmConstantinople(1);
    manager.getDynamicPropertiesStore().saveAllowLvmSolidity059(0);

    String contractName = "KandyOfTransferFailedTest";
    ...
    Transaction usdl = LvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    byte[] addressWithSufficientBalance = WalletUtil.generateContractAddress(usdl);
    runtime = LvmTestUtils.processTransactionAndReturnRuntime(usdl, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());

    for (TestCase testCase : testCasesAfterAllowLvmConstantinop) {
      checkResult(testCase, addressWithSufficientBalance);
    }

    usdl = LvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, address, ABI, code, 0, fee, consumeUserResourcePercent,
        null);
    byte[] addressWithoutBalance = WalletUtil.generateContractAddress(usdl);
    runtime = LvmTestUtils.processTransactionAndReturnRuntime(usdl, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
  ...
  @Test
  public void testTransferFailedBeforeAllowLvmConstantinopl()
      throws ContractExeException, ReceiptCheckErrException,
      VMIllegalException, ContractValidateException {
    ConfigLoader.disable = false;
    manager.getDynamicPropertiesStore().saveAllowLvmTransferTrc10(1);
    manager.getDynamicPropertiesStore().saveAllowLvmConstantinople(0);
    manager.getDynamicPropertiesStore().saveAllowLvmSolidity059(0);
    ...
    for (TestCase testCase : testCasesBeforeAllowLvmConstantinop) {
      checkResult(testCase, addressWithSufficientBalance);
    }
  ...
  private void checkResult(TestCase testCase, byte[] factoryAddress)
    ...
    Assert.assertEquals(receiptCapsule.getResult(), testCase.getReceiptResult(),
        testCase.getMethod());
    if (testCase.allKandy) {
      Assert.assertEquals(programResult.getKandyUsed(), 1000000, testCase.getMethod());
    } else {
      Assert.assertTrue(programResult.getKandyUsed() < allKandy, testCase.getMethod());
    }
  }

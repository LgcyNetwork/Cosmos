...
public class BandWidthRuntimeWithCheckTest {
  ...
  public static void init() {
    ...
    accountCapsule2.setFrozenForKandy(10_000_000L, 0L);
    dbManager.getAccountStore()
    ...
    accountCapsule3.setFrozenForKandy(10_000_000L, 0L);
    dbManager.getAccountStore()
  ...
  public void testSuccess() {
    try {
      ...
      long kandy = triggerOwner.getKandyUsage();
      ...
      TransactionCapsule usdlCap = new TransactionCapsule(transaction);
      ...
      triggerOwner = dbManager.getAccountStore()
          .get(Commons.decodeFromBase58Check(TriggerOwnerAddress));
      kandy = triggerOwner.getKandyUsage() - kandy;
      balance = balance - triggerOwner.getBalance();
      Assert.assertEquals(624668, trace.getReceipt().getKandyUsageTotal());
      Assert.assertEquals(50000, kandy);
      Assert.assertEquals(57466800, balance);
      Assert.assertEquals(624668 * Constant.SUN_PER_KANDY,
          balance + kandy * Constant.SUN_PER_KANDY);
    ...
    try {
      ...
      Transaction transaction = Transaction.newBuilder().setRawData(raw.newBuilder().addContract(
          Contract.newBuilder().setParameter(Any.pack(triggerContract))
              .setType(ContractType.TriggerSmartContract)).setFeeLimit(1000000000)).build();
      TransactionCapsule usdlCap = new TransactionCapsule(transaction);
      usdlCap.setResultCode(contractResult.SUCCESS);
      TransactionTrace trace = new TransactionTrace(usdlCap, StoreFactory.getInstance(),
          new RuntimeImpl());
      BlockCapsule blockCapsule = null;
      DepositImpl deposit = DepositImpl.createRoot(dbManager);
      ...
      Assert.assertEquals(6118, receipt.getKandyUsageTotal());
      Assert.assertEquals(6118, receipt.getKandyUsage());
      Assert.assertEquals(0, receipt.getKandyFee());
  ...
  private byte[] createContract()
    ...
    long kandy = owner.getKandyUsage();
    ...
    owner = dbManager.getAccountStore()
        .get(Commons.decodeFromBase58Check(OwnerAddress));
    kandy = owner.getKandyUsage() - kandy;
    balance = balance - owner.getBalance();
    Assert.assertNull(trace.getRuntimeError());
    Assert.assertEquals(88529, trace.getReceipt().getKandyUsageTotal());
    Assert.assertEquals(50000, kandy);
    Assert.assertEquals(3852900, balance);
    Assert
        .assertEquals(88529 * Constant.SUN_PER_KANDY,
            balance + kandy * Constant.SUN_PER_KANDY);

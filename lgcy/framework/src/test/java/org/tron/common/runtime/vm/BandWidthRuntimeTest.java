...
public class BandWidthRuntimeTest {
  ...
  public void testSuccess() {
    try {
      ...
      triggerOwner = dbManager.getAccountStore()
          .get(Commons.decodeFromBase58Check(TriggerOwnerAddress));
      kandy = triggerOwner.getKandyUsage();
      long balance = triggerOwner.getBalance();
      Assert.assertEquals(45706, trace.getReceipt().getKandyUsageTotal());
      Assert.assertEquals(45706, kandy);
  ...
  public void testSuccessNoBandd() {
    try {
      ...
      Assert.assertEquals(522850, receipt.getKandyUsageTotal());
      Assert.assertEquals(50000, receipt.getKandyUsage());
      Assert.assertEquals(47285000, receipt.getKandyFee());
      Assert.assertEquals(totalBalance - receipt.getKandyFee(),
          balance);
    } catch (TronException e) {
      Assert.assertNotNull(e);
    }
  ...
  private byte[] createContract()
    ...
    Assert.assertEquals(52299, trace.getReceipt().getKandyUsageTotal());
    Assert.assertEquals(50000, kandy);
    Assert.assertEquals(229900, balance);
    Assert
        .assertEquals(52299 * Constant.SUN_PER_KANDY,

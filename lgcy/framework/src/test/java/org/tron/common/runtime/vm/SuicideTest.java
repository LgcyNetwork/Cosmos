...
*/
  ...
  public void testSuicide() throws Exception {
    ...
    usdl = LvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, ownerAddress, abi, factoryCode, 100000000, feeLimit, 0,
        null);
    factoryAddress = WalletUtil.generateContractAddress(usdl);
    runtime = LvmTestUtils.processTransactionAndReturnRuntime(usdl, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
    ...
    usdl = LvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    byte[] obtainContractAddr = WalletUtil.generateContractAddress(usdl);
    ...
    Assert.assertEquals(blackHoleAccountAfter.getBalance(), 200000000
        + blackHoleAccount.getBalance() + 25500); // 25500 for kandy used for suicide
    ...
    usdl = LvmTestUtils.generateDeploySmartContractAndGetTransaction(
        "contractObtain", ownerAddress, abi, factoryCode, 100000000, feeLimit, 0,
        null);
    obtainContractAddr = WalletUtil.generateContractAddress(usdl);
    obtainContractAddrStr = StringUtil.encode58Check(obtainContractAddr);
    runtime = LvmTestUtils.processTransactionAndReturnRuntime(usdl, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
    //deploy contract2
    usdl = LvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    byte[] suicideContractAddr = WalletUtil.generateContractAddress(usdl);

...
public class TransferToAccountTest {
  ...
  private static final int USDL_NUM = 10;
  private static final int NUM = 1;
  private static final long START_TIME = 1;
  private static final long END_TIME = 2;
  private static final int VOTE_SCORE = 2;
  private static final String DESCRIPTION = "USDL";
  ...
  private long createAsset(String tokenName) {
    ...
    chainBaseManager.getDynamicPropertiesStore().saveAllowLvmTransferTrc10(1);
    chainBaseManager.getDynamicPropertiesStore().saveAllowLvmConstantinople(1);
    chainBaseManager.getDynamicPropertiesStore().saveAllowLvmSolidity059(1);
  ...
  public void TransferTokenTest()
    ...
    runtime = LvmTestUtils.processTransactionAndReturnRuntime(transaction, dbManager, null);
    ...
    long kandyCostWhenExist = runtime.getResult().getKandyUsed();
    ...
    Assert.assertEquals(9,
        chainBaseManager.getAccountStore().get(ecKey.getAddress()).getAssetMapV2()
            .get(String.valueOf(id)).longValue());
    long kandyCostWhenNonExist = runtime.getResult().getKandyUsed();
    //4.Test Kandy
    Assert.assertEquals(kandyCostWhenNonExist - kandyCostWhenExist,
        KandyCost.getInstance().getNEW_ACCT_CALL());
    //5. Test transfer USDL with exsit account
    ...
    kandyCostWhenExist = runtime.getResult().getKandyUsed();

    //6. Test  transfer USDL with non-exsit account
    selectorStr = "transferTo(address,uint256)";
    ...
    kandyCostWhenNonExist = runtime.getResult().getKandyUsed();

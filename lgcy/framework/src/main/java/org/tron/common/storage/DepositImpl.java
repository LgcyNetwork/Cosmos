...
public class DepositImpl implements Deposit {
  ...
  public synchronized Storage getStorage(byte[] address) {
    ...
    ContractCapsule contract = getContract(address);
    if (contract != null && !ByteUtil.isNullOrZeroArray(contract.getUsdlHash())) {
      storage.generateAddrHash(contract.getUsdlHash());
    }
    return storage;
  }
  ...
  @Override
  public TransactionCapsule getTransaction(byte[] usdlHash) {
    Key key = Key.create(usdlHash);
    ...
    if (parent != null) {
      transactionCapsule = parent.getTransaction(usdlHash);
    } else {
      try {
        transactionCapsule = getTransactionStore().get(usdlHash);

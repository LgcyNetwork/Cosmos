...
public class RuntimeImpl implements Runtime {
  ...
  public void execute(TransactionContext context)
    ...
    ContractType contractType = context.getUsdlCap().getInstance().getRawData().getContract(0)
        .getType();
    switch (contractType.getNumber()) {
      ...
      case ContractType.CreateSmartContract_VALUE:
        ...
        break;
      default:
        actuatorList = ActuatorCreator.getINSTANCE().createActuator(context.getUsdlCap());
    }
    if (actuator2 != null) {
  ...
  private void setResultCode(ProgramResult result) {
    ...
    if (exception instanceof OutOfKandyException) {
      result.setResultCode(contractResult.OUT_OF_USDL);
      return;
    }

...
public class FetchInvDataMsgHandler implements TronMsgHandler {
  ...
  private void check(PeerConnection peer, FetchInvDataMessage fetchInvDataMsg) throws P2pException {
    ...
    if (type == MessageTypes.USDL) {
      for (Sha256Hash hash : fetchInvDataMsg.getHashList()) {
        if (peer.getAdvInvSpread().getIfPresent(new Item(hash, InventoryType.USDL)) == null) {
          throw new P2pException(TypeEnum.BAD_MESSAGE, "not spread inv: {}" + hash);
        }
      }
      int fetchCount = peer.getNodeStatistics().messageStatistics.tronInUSDLFetchInvDataElement
          .getCount(10);
      int maxCount = advService.getUsdlCount().getCount(60);

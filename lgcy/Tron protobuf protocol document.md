...
- message `Account` has multiple attributes and 2 nested messages: 
  ...
  message AccountResource {
    int64 kandy_usage = 1;
    Frozen frozen_balance_for_power = 2;
    int64 latest_consume_time_for_power = 3;
    int64 acquired_delegated_frozen_balance_for_kandy = 4;
    int64 delegated_frozen_balance_for_kandy = 5;
  ...
  `balance`: the USDL balance of this account.

  `votes`: received votes of this account. – e.g. *{(“0x1b7w…9xj3”,323), (“0x8djq…j12m”,88),…,(“0x82nd…mx6i”,10001)}*.

  `asset`: other assets except USDL in this account – e.g. *{<“WishToken”,66666>,<”Dogie”,233>}*.

  `assetV2`: other assets except USDL in this account – e.g. *{<“WishToken”,66666>,<”Dogie”,233>}*. (used after allowing same name of token87)

  `frozen`: the freezed USDL of this account for receiving bandwidth
  ...
  `latest_consume_time`: the latest consume kandy time of this account.
...
- Any behaviors which consume kandy are regarded as transaction.
...
- message `TransactionRet`
  ...
  - message `ResourceReceipt`
    ...
    `kandy_usage`: consume yourself account kandy.

    `kandy_fee`: consume yourself account fee.

    `origin_kandy_usage`: consume contract owner account kandy.

    `kandy_usage_total`: consume total account fee.

    `net_usage`: consume yourself net.

    `net_fee`: consume yourself usdl of net usage.
    ...
    message ResourceReceipt {
      int64 kandy_usage = 1;
      int64 kandy_fee = 2;
      int64 origin_kandy_usage = 3;
      int64 kandy_usage_total = 4;
      int64 net_usage = 5;
      int64 net_fee = 6;
  ...
  - Message `InternalTransaction`
    ...
    `callValueInfo`: Refers to asset transfer information in internal transactions, including usdl and trc10.
  ...
  message `raw`
   ...
   `fee_limit`: the cost limit for kandy and fee when trigger and create contract.
  ...
  - message `TransactionInfo`
   ...
   `receipt`: the receipt of fee and kandy usage. 
   ...
   `unfreeze_amount`: unfreeze usdl amount.
...
- message `Contract`
  ...
  `provider`: reservedUpdateKandyLimitContract.
...
- There are 15 types of results while deploying contracts (refer to `Transaction.Result`):
  ...
  - message `VoteWitnessContract`
    ...
    - message `AssetIssueContract`
      ...
      `usdl_num`: usdl num defines token price.

      `precision`: precision.

      `num`: usdl num defines token price.
    ...
    - message `FreezeBalanceContract`
      ...
      `frozen_balance`: frozen amount of USDL.

      `frozen_duration`: frozen duration of USDL.

      `resource`: type of resource gained from freezing USDL.
    ...
    - message `UnfreezeBalanceContract`
      ...
      `resource`: type of resource, BANDWIDTH / USDL_POWER. 
     ...
     - message `TriggerSmartContract`
       ...
       `call_value`: USDL amount sent to smart contract.
     ...
     - message `UpdateSettingContract`
       ...
       `consume_user_resource_percent`: user kandy payment percentage of whole kandy payment includes contract deployer’s and user's kandy payment.
     ...
     - message `UpdateKandyLimitContract`:
    
       `owner_address`: address of owner.
    
       `contract_address`: smart contract address.
    
       `origin_kandy_limit`: value of owner’s consume kandy limit for each transaction.
    
      ```java
      message UpdateKandyLimitContract {
          bytes owner_address = 1;
          bytes contract_address = 2;
          int64 origin_kandy_limit = 3;
      }
      ```
...
- message `SmartContract`
  ...
  `call_value`: amount of USDL that send to the smart contract.
  
  `consume_user_resource_percent`: user kandy payment percentage of the whole kandy payment which includes both contract deployer’s payment and user kandy payment.
  
  `name`: the name of the smart contract.
  
  `origin_kandy_limit`: value of the owner’s consume kandy limit for each transaction.
  
  `code_hash`: hash of smart contract bytecode.
  
  `usdl_hash`:  transactionId of Deploying contract transaction.
...
- #### Node Information
  ...
  - message `NodeInfo`
    ...
    - message `ConfigNodeInfo`:
      ...
      `allowAdaptiveKandy`: permission of turning on adaptive kandy.

...
import static org.tron.common.runtime.InternalTransaction.USDLType.USDL_CONTRACT_CALL_TYPE;
import static org.tron.common.runtime.InternalTransaction.USDLType.USDL_CONTRACT_CREATION_TYPE;
...
public class TransactionTrace {
  ...
  private KandyProcessor kandyProcessor;

  private USDLType usdlType;
  ...
  public TransactionTrace(TransactionCapsule usdl, StoreFactory storeFactory,
      Runtime runtime) {
    this.usdl = usdl;
    Transaction.Contract.ContractType contractType = this.usdl.getInstance().getRawData()
        .getContract(0).getType();
    switch (contractType.getNumber()) {
      case ContractType.TriggerSmartContract_VALUE:
        usdlType = USDL_CONTRACT_CALL_TYPE;
        break;
      case ContractType.CreateSmartContract_VALUE:
        usdlType = USDL_CONTRACT_CREATION_TYPE;
        break;
      default:
        usdlType = USDLType.USDL_PRECOMPILED_TYPE;
    }
    ...
    this.kandyProcessor = new KandyProcessor(dynamicPropertiesStore, accountStore);
  ...
  public TransactionCapsule getUSDL() {
    return usdl;
  }

  private boolean needVM() {
    return this.usdlType == USDL_CONTRACT_CALL_TYPE
        || this.usdlType == USDL_CONTRACT_CREATION_TYPE;
  }
  ...
  public void init(BlockCapsule blockCap, boolean eventPluginLoaded) {
    ...
    transactionContext = new TransactionContext(blockCap, usdl, storeFactory, false,
        eventPluginLoaded);
  }

  public void checkIsConstant() throws ContractValidateException, VMIllegalException {
    if (dynamicPropertiesStore.getAllowLvmConstantinople() == 1) {
      return;
    }
    TriggerSmartContract triggerContractFromTransaction = ContractCapsule
        .getTriggerContractFromTransaction(this.getUSDL().getInstance());
    if (USDL_CONTRACT_CALL_TYPE == this.usdlType) {
  ...
  //set bill
  public void setBill(long kandyUsage) {
    if (kandyUsage < 0) {
      kandyUsage = 0L;
    }
    receipt.setKandyUsageTotal(kandyUsage);
  }
  ...
  public void exec()
    ...
    setBill(transactionContext.getProgramResult().getKandyUsed());

    if (USDLType.USDL_PRECOMPILED_TYPE != usdlType) {
  ...
  /**
   * pay actually bill(include USDL_POWER and storage).
   */
  public void pay() throws BalanceInsufficientException {
    byte[] originAccount;
    byte[] callerAccount;
    long percent = 0;
    long originKandyLimit = 0;
    switch (usdlType) {
      case USDL_CONTRACT_CREATION_TYPE:
        callerAccount = TransactionCapsule.getOwner(usdl.getInstance().getRawData().getContract(0));
        originAccount = callerAccount;
        break;
      case USDL_CONTRACT_CALL_TYPE:
        TriggerSmartContract callContract = ContractCapsule
            .getTriggerContractFromTransaction(usdl.getInstance());
        ...
        originKandyLimit = contractCapsule.getOriginKandyLimit();
        break;
      default:
        return;
    }
    ...
    receipt.payKandyBill(
        dynamicPropertiesStore, accountStore, forkController,
        origin,
        caller,
        percent, originKandyLimit,
        kandyProcessor,
        KandyProcessor.getHeadSlot(dynamicPropertiesStore));
  }
  ...
  public boolean checkNeedRetry() {
    ...
    return usdl.getContractRet() != contractResult.OUT_OF_TIME && receipt.getResult()
  ...
  public void check() throws ReceiptCheckErrException {
    ...
    if (Objects.isNull(usdl.getContractRet())) {
      throw new ReceiptCheckErrException("null resultCode");
    }
    if (!usdl.getContractRet().equals(receipt.getResult())) {
      logger.info(
          "this tx id: {}, the resultCode in received block: {}, the resultCode in self: {}",
          Hex.toHexString(usdl.getTransactionId().getBytes()), usdl.getContractRet(),
          receipt.getResult());

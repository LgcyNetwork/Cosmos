...
public class TransactionUtil {
  ...
  public static TransactionInfoCapsule buildTransactionInfoInstance(TransactionCapsule usdlCap,
    ...
    builder.setId(ByteString.copyFrom(usdlCap.getTransactionId().getBytes()));
    ProgramResult programResult = trace.getRuntimeResult();
    long fee =
        programResult.getRet().getFee() + traceReceipt.getKandyFee()
    ...
    if (CommonParameter.getInstance().isSaveInternalTx() && null != programResult
      ...
      for (InternalTransaction internalTransaction : programResult
          .getInternalTransactions()) {
        Protocol.InternalTransaction.Builder internalUSDLBuilder = Protocol.InternalTransaction
            .newBuilder();
        // set hash
        internalUSDLBuilder.setHash(ByteString.copyFrom(internalTransaction.getHash()));
        // set caller
        internalUSDLBuilder.setCallerAddress(ByteString.copyFrom(internalTransaction.getSender()));
        // set TransferTo
        internalUSDLBuilder
        ...
        // usdl will not be set token name
        callValueInfoBuilder.setCallValue(internalTransaction.getValue());
        // Just one transferBuilder for now.
        internalUSDLBuilder.addCallValueInfo(callValueInfoBuilder);
        internalTransaction.getTokenInfo().forEach((tokenId, amount) -> {
          internalUSDLBuilder.addCallValueInfo(
              Protocol.InternalTransaction.CallValueInfo.newBuilder().setTokenId(tokenId)
                  .setCallValue(amount));
        });
        // Token for loop end here
        internalUSDLBuilder.setNote(ByteString.copyFrom(internalTransaction.getNote().getBytes()));
        internalUSDLBuilder.setRejected(internalTransaction.isRejected());
        builder.addInternalTransactions(internalUSDLBuilder);
      }
    }

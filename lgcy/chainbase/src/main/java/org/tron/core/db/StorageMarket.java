...
public class StorageMarket {
  ...
  private long exchangeToSupply(boolean isUSDL, long quant) {
    logger.info("isUSDL: " + isUSDL);
    long balance = isUSDL ? dynamicPropertiesStore.getTotalStoragePool() :
...
//    }
  ...
  private long exchangeToSupply2(boolean isUSDL, long quant) {
    logger.info("isUSDL: " + isUSDL);
    long balance = isUSDL ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    logger.info("balance: " + balance);
...
//    }
  ...
  private long exchange_from_supply(boolean isUSDL, long supplyQuant) {
    long balance = isUSDL ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    supply -= supplyQuant;
  ...
  public long exchange(long from, boolean isUSDL) {
    long relay = exchangeToSupply(isUSDL, from);
    return exchange_from_supply(!isUSDL, relay);
  }

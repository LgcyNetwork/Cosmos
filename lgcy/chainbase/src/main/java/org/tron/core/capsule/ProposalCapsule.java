...
public class ProposalCapsule implements ProtoCapsule<Proposal> {
  ...
  public boolean hasMostApprovals(List<ByteString> activeWitnesses) {
    long count = this.proposal.getApprovalsList().stream()
        ...
        logger.info("test capsule");
        logger.info(String.valueOf(count));
        logger.info(String.valueOf(this.proposal.getApprovalsCount()));
    ...
    List<ByteString> list1= Lists.newArrayList();
    List<ByteString> list2= Lists.newArrayList();
    List<ByteString> list3= Lists.newArrayList();

    for (int i = 0; i <=8; i++) {
      list1.add(activeWitnesses.get(i));
    }
    // System.out.println(list1);

    long count1 = this.proposal.getApprovalsList().stream().filter(witness -> list1.contains(witness)).count();
    ...
    for (int i = 9; i <=17; i++) {
        list2.add(activeWitnesses.get(i));
    }
    // System.out.println(list2);

    long count2 = this.proposal.getApprovalsList().stream().filter(witness -> list2.contains(witness)).count();
    ...
    for (int i = 18; i <=26; i++) {
      list3.add(activeWitnesses.get(i));
    }
    // System.out.println(list3);

    long count3 = this.proposal.getApprovalsList().stream().filter(witness -> list3.contains(witness)).count();
    logger.info(String.valueOf(count3));
    logger.info("test3");

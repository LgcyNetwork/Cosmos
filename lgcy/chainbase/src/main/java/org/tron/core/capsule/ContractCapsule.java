...
public class ContractCapsule implements ProtoCapsule<SmartContract> {
  ...
  public static CreateSmartContract getSmartContractFromTransaction(Transaction usdl) {
    try {
  ...
  public static TriggerSmartContract getTriggerContractFromTransaction(Transaction usdl) {
    try {
      Any any = usdl.getRawData().getContract(0).getParameter();
  ...
  public long getOriginKandyLimit() {
    long originKandyLimit = this.smartContract.getOriginKandyLimit();
    if (originKandyLimit == Constant.PB_DEFAULT_KANDY_LIMIT) {
      originKandyLimit = Constant.CREATOR_DEFAULT_KANDY_LIMIT;
    }
    return originKandyLimit;
  }
  ...
  public byte[] getUsdlHash() {
    return this.smartContract.getUsdlHash().toByteArray();
  }

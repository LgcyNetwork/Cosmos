...
public class ForkController {
  ...
  private boolean passOld(int version) {
    if (version == ForkBlockVersionConsts.KANDY_LIMIT) {
      return checkForKandyLimit();
    }
  ...
  // it make block use new kandy to handle transaction when block number >= 4727890L.
  // version !=5, skip this.
  private boolean checkForKandyLimit() {
    long blockNum = manager.getDynamicPropertiesStore().getLatestBlockHeaderNumber();
    return blockNum >= CommonParameter.getInstance().getBlockNumForKandyLimit();
  }

...
public class InternalTransaction {
  ...
  /* the amount of usdl to transfer (calculated as sun) */
  private long value;
   ...
   */
  public InternalTransaction(Transaction usdl, InternalTransaction.USDLType usdlType)
      throws ContractValidateException {
    this.transaction = usdl;
    TransactionCapsule usdlCap = new TransactionCapsule(usdl);
    this.protoEncoded = usdlCap.getData();
    this.nonce = 0;
    ...
    this.deep = -1;
    if (usdlType == USDLType.USDL_CONTRACT_CREATION_TYPE) {
      ...
      this.transferToAddress = WalletUtil.generateContractAddress(usdl);
      this.note = "create";
    ...
    } else if (usdlType == USDLType.USDL_CONTRACT_CALL_TYPE) {
    ...
    this.hash = usdlCap.getTransactionId().getBytes();
  }
  ...
  public InternalTransaction(byte[] parentHash, int deep, int index,
    ...
    // in this case, value also can represent a tokenValue when tokenId is not null, otherwise it is a usdl callvalue.
    this.value = value;
    this.data = ArrayUtils.nullToEmpty(data);
    this.nonce = nonce;
    this.hash = getHash();
    // in a contract call contract case, only one value should be used. usdl or a token. can't be both. We should avoid using
  ...
  public enum USDLType {
    USDL_PRECOMPILED_TYPE,
    USDL_CONTRACT_CREATION_TYPE,
    USDL_CONTRACT_CALL_TYPE,
    USDL_UNKNOWN_TYPE,
  }

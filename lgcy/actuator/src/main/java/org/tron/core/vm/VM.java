...
import org.tron.core.vm.program.Program.OutOfKandyException;
...
public class VM {
  ...
  private static final String KANDY_LOG_FORMATE = "{} Op:[{}]  Kandy:[{}] Deep:[{}] Hint:[{}]";
  // 3MB
  ...
  private long calcMemKandy(KandyCost kandyCosts, long oldMemSize, BigInteger newMemSize,
    ...
    if (memoryUsage > oldMemSize) {
      ...
      long memKandy = (kandyCosts.getMEMORY() * memWords + memWords * memWords / 512)
          - (kandyCosts.getMEMORY() * memWordsOld + memWordsOld * memWordsOld / 512);
      kandyCost += memKandy;
    }

    if (copySize > 0) {
      long copyKandy = kandyCosts.getCOPY_KANDY() * ((copySize + 31) / 32);
      kandyCost += copyKandy;
    }
    return kandyCost;
  }
  ...
  public void step(Program program) {
    ...
    try {
      ...
      if (!VMConfig.allowLvmSolidity059() && op == ISCONTRACT) {
        throw Program.Exception.invalidOpCode(program.getCurrentOp());
      }

      if (!VMConfig.allowLvmIstanbul() && (op == SELFBALANCE || op == CHAINID)) {
        throw Program.Exception.invalidOpCode(program.getCurrentOp());
      }
      ...
      if(!VMConfig.allowLvmAssetIssue() && (op == TOKENISSUE || op == UPDATEASSET)) {
        throw Program.Exception.invalidOpCode(program.getCurrentOp());
      }
      ...
      String hint = "";
      long kandyCost = op.getTier().asInt();
      KandyCost kandyCosts = KandyCost.getInstance();
      DataWord adjustedCallKandy = null;

      // Calculate fees and spend kandy
      switch (op) {
        case STOP:
          kandyCost = kandyCosts.getSTOP();
          break;
        case SUICIDE:
          kandyCost = kandyCosts.getSUICIDE();
          ...
          if (isDeadAccount(program, suicideAddressWord)
            ...
            kandyCost += kandyCosts.getNEW_ACCT_SUICIDE();
          }
          break;
        case SSTORE:
          ...
          if (oldValue == null && !newValue.isZero()) {
            ...
            kandyCost = kandyCosts.getSET_SSTORE();
          } else if (oldValue != null && newValue.isZero()) {
            // set zero to an old value
            program.futureRefundKandy(kandyCosts.getREFUND_SSTORE());
            kandyCost = kandyCosts.getCLEAR_SSTORE();
          } else {
            // include:
            // [1] oldValue == null && newValue == 0
            // [2] oldValue != null && newValue != 0
            kandyCost = kandyCosts.getRESET_SSTORE();
          }
          break;
        case SLOAD:
          kandyCost = kandyCosts.getSLOAD();
          break;
        case TOKENBALANCE:
        case BALANCE:
        case REWARDBALANCE:
        case ISCONTRACT:
        case ISSRCANDIDATE:
          kandyCost = kandyCosts.getBALANCE();
          break;

        // These all operate on memory and therefore potentially expand it:
        case MSTORE:
          kandyCost = calcMemKandy(kandyCosts, oldMemSize,
        ...
        case RETURN:
        case REVERT:
          kandyCost = kandyCosts.getSTOP() + calcMemKandy(kandyCosts, oldMemSize,
              memNeeded(stack.peek(), stack.get(stack.size() - 2)), 0, op);
          break;
        case SHA3:
          kandyCost = kandyCosts.getSHA3() + calcMemKandy(kandyCosts, oldMemSize,
              memNeeded(stack.peek(), stack.get(stack.size() - 2)), 0, op);
          DataWord size = stack.get(stack.size() - 2);
          long chunkUsed = (size.longValueSafe() + 31) / 32;
          kandyCost += chunkUsed * kandyCosts.getSHA3_WORD();
          break;
        case CALLDATACOPY:
        case RETURNDATACOPY:
          kandyCost = calcMemKandy(kandyCosts, oldMemSize,
        ...
        case CODECOPY:
          ...
          break;
        case EXTCODESIZE:
          kandyCost = kandyCosts.getEXT_CODE_SIZE();
          break;
        case EXTCODECOPY:
          kandyCost = kandyCosts.getEXT_CODE_COPY() + calcMemKandy(kandyCosts, oldMemSize,
              memNeeded(stack.get(stack.size() - 2), stack.get(stack.size() - 4)),
              stack.get(stack.size() - 4).longValueSafe(), op);
          break;
        case EXTCODEHASH:
          kandyCost = kandyCosts.getEXT_CODE_HASH();
          break;
        case CALL:
        case CALLCODE:
        case DELEGATECALL:
        ...
        case CALLTOKEN:
          ...
          kandyCost = kandyCosts.getCALL();
          DataWord callKandyWord = stack.get(stack.size() - 1);
          ...
          if ((op == CALL || op == CALLTOKEN)
              ...
              && !value.isZero()) {
            kandyCost += kandyCosts.getNEW_ACCT_CALL();
          }

          // TODO #POC9 Make sure this is converted to BigInteger (256num support)
          if (!value.isZero()) {
            kandyCost += kandyCosts.getVT_CALL();
          }
          ...
          kandyCost += calcMemKandy(kandyCosts, oldMemSize, in.max(out), 0, op);
          checkMemorySize(op, in.max(out));

          if (kandyCost > program.getKandyLimitLeft().longValueSafe()) {
            throw new OutOfKandyException(
                "Not enough kandy for '%s' operation executing: opKandy[%d], programKandy[%d]",
                op.name(),
                kandyCost, program.getKandyLimitLeft().longValueSafe());
          }
          DataWord getKandyLimitLeft = program.getKandyLimitLeft().clone();
          getKandyLimitLeft.sub(new DataWord(kandyCost));

          adjustedCallKandy = program.getCallKandy(op, callKandyWord, getKandyLimitLeft);
          kandyCost += adjustedCallKandy.longValueSafe();
          break;
        case CREATE:
          kandyCost = kandyCosts.getCREATE() + calcMemKandy(kandyCosts, oldMemSize,
              memNeeded(stack.get(stack.size() - 2), stack.get(stack.size() - 3)), 0, op);
          break;
        case CREATE2:
          DataWord codeSize = stack.get(stack.size() - 3);
          kandyCost = kandyCosts.getCREATE();
          kandyCost += calcMemKandy(kandyCosts, oldMemSize,
              memNeeded(stack.get(stack.size() - 2), stack.get(stack.size() - 3)), 0, op);
          kandyCost += DataWord.sizeInWords(codeSize.intValueSafe()) * kandyCosts.getSHA3_WORD();
        ...
        case LOG4:
          ...
          BigInteger dataCost = dataSize
              .multiply(BigInteger.valueOf(kandyCosts.getLOG_DATA_KANDY()));
          if (program.getKandyLimitLeft().value().compareTo(dataCost) < 0) {
            throw new OutOfKandyException(
                "Not enough kandy for '%s' operation executing: opKandy[%d], programKandy[%d]",
                op.name(),
                dataCost.longValueExact(), program.getKandyLimitLeft().longValueSafe());
          }
          kandyCost = kandyCosts.getLOG_KANDY()
              + kandyCosts.getLOG_TOPIC_KANDY() * nTopics
              + kandyCosts.getLOG_DATA_KANDY() * stack.get(stack.size() - 2).longValue()
              + calcMemKandy(kandyCosts, oldMemSize,
              memNeeded(stack.peek(), stack.get(stack.size() - 2)), 0, op);
        ...
        case EXP:
          ...
          kandyCost =
              (long) kandyCosts.getEXP_KANDY() + kandyCosts.getEXP_BYTE_KANDY() * bytesOccupied;
          break;
        case STAKE:
        case UNSTAKE:
          kandyCost = kandyCosts.getStakeAndUnstake();
          break;
        case WITHDRAWREWARD:
          kandyCost = kandyCosts.getWithdrawReward();
          break;
        case TOKENISSUE:
          kandyCost = kandyCosts.getTokenIssue();
          break;
        case UPDATEASSET:
          kandyCost = kandyCosts.getUpdateAsset();
          break;
        default:
          break;
      }

      program.spendKandy(kandyCost, op.name());
      ...
      switch (op) {
        ...
        */
        ...
        break;
        case GASPRICE: {
          DataWord kandyPrice = new DataWord(0);

          if (logger.isDebugEnabled()) {
            hint = "price: " + kandyPrice.toString();
          }

          program.stackPush(kandyPrice);
          program.step();
        }
        break;
        ...
        break;
        case GASLIMIT: {
          // todo: this kandylimit is the block's kandy limit
          DataWord kandyLimit = new DataWord(0);

          if (logger.isDebugEnabled()) {
            hint = "kandylimit: " + kandyLimit;
          }

          program.stackPush(kandyLimit);
          program.step();
        }
        break;
        case CHAINID: {
        ...
        break;
        case GAS: {
          DataWord kandy = program.getKandyLimitLeft();
          if (logger.isDebugEnabled()) {
            hint = "" + kandy;
          }

          program.stackPush(kandy);
          program.step();
        }
        break;
        ...
        case DELEGATECALL:
        case STATICCALL: {
          program.stackPop(); // use adjustedCallKandy instead of requested
          ...
          if (!value.isZero()) {
            adjustedCallKandy.add(new DataWord(kandyCosts.getSTIPEND_CALL()));
          }
          ...
          if (logger.isDebugEnabled()) {
            hint = "addr: " + Hex.toHexString(codeAddress.getLast20Bytes())
                + " kandy: " + adjustedCallKandy.shortHex()
                + " inOff: " + inDataOffs.shortHex()
                + " inSize: " + inDataSize.shortHex();
            logger.debug(KANDY_LOG_FORMATE, String.format("%5s", "[" + program.getPC() + "]"),
                String.format("%-12s", op.name()),
                program.getKandyLimitLeft().value(),
          ...
          MessageCall msg = new MessageCall(
              op, adjustedCallKandy, codeAddress, value, inDataOffs, inDataSize,

...
public class Storage {
  ...
  private static byte[] addrHash(byte[] address, byte[] usdlHash) {
    if (ByteUtil.isNullOrZeroArray(usdlHash)) {
      return Hash.sha3(address);
    }
    return Hash.sha3(ByteUtil.merge(address, usdlHash));
  }

  public void generateAddrHash(byte[] usdlId) {
    // update addreHash for create2
    addrHash = addrHash(address, usdlId);
  }

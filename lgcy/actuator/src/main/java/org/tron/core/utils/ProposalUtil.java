...
public class ProposalUtil {
  ...
  public static void validator(DynamicPropertiesStore dynamicPropertiesStore,
    ...
    switch (proposalType) {
      ...
      case TOTAL_KANDY_LIMIT: { // deprecated
        if (!forkController.pass(ForkBlockVersionConsts.KANDY_LIMIT)) {
      ...
      case ALLOW_LVM_TRANSFER_TRC10: {
        if (value != 1) {
          throw new ContractValidateException(
              PRE_VALUE_NOT_ONE_ERROR + "ALLOW_LVM_TRANSFER_TRC10" + VALUE_NOT_ONE_ERROR);
        }
        if (dynamicPropertiesStore.getAllowSameTokenName() == 0) {
          throw new ContractValidateException("[ALLOW_SAME_TOKEN_NAME] proposal must be approved "
              + "before [ALLOW_LVM_TRANSFER_TRC10] can be proposed");
        }
        break;
      }
      case TOTAL_CURRENT_KANDY_LIMIT: {
        if (!forkController.pass(ForkBlockVersionEnum.VERSION_3_2_2)) {
          throw new ContractValidateException(BAD_PARAM_ID);
        }
        if (value < 0 || value > LONG_VALUE) {
      ...
      case ALLOW_ADAPTIVE_KANDY: {
        if (!forkController.pass(ForkBlockVersionEnum.VERSION_3_5)) {
          throw new ContractValidateException("Bad chain parameter id: ALLOW_ADAPTIVE_KANDY");
        }
        if (value != 1) {
          throw new ContractValidateException(
              PRE_VALUE_NOT_ONE_ERROR + "ALLOW_ADAPTIVE_KANDY" + VALUE_NOT_ONE_ERROR);
        }
        break;
      }
      ...
      case ALLOW_LVM_CONSTANTINOPLE: {
        if (!forkController.pass(ForkBlockVersionEnum.VERSION_3_6)) {
          throw new ContractValidateException(BAD_PARAM_ID);
        }
        if (value != 1) {
          throw new ContractValidateException(
              PRE_VALUE_NOT_ONE_ERROR + "ALLOW_LVM_CONSTANTINOPLE" + VALUE_NOT_ONE_ERROR);
        }
        if (dynamicPropertiesStore.getAllowLvmTransferTrc10() == 0) {
          throw new ContractValidateException(
              "[ALLOW_LVM_TRANSFER_TRC10] proposal must be approved "
                  + "before [ALLOW_LVM_CONSTANTINOPLE] can be proposed");
        }
        break;
      }
      case ALLOW_LVM_SOLIDITY_059: {
        ...
        if (value != 1) {
          throw new ContractValidateException(
              PRE_VALUE_NOT_ONE_ERROR + "ALLOW_LVM_SOLIDITY_059" + VALUE_NOT_ONE_ERROR);
        }
        if (dynamicPropertiesStore.getAllowCreationOfContracts() == 0) {
          throw new ContractValidateException(
              "[ALLOW_CREATION_OF_CONTRACTS] proposal must be approved "
                  + "before [ALLOW_LVM_SOLIDITY_059] can be proposed");
        }
        break;
      }
...
//      }
      ...
      case ALLOW_LVM_ISTANBUL: {
        if (!forkController.pass(ForkBlockVersionEnum.VERSION_4_1)) {
          throw new ContractValidateException(
              "Bad chain parameter id [ALLOW_LVM_ISTANBUL]");
        }
        if (value != 1) {
          throw new ContractValidateException(
              "This value[ALLOW_LVM_ISTANBUL] is only allowed to be 1");
        }
        break;
      }
      ...
      // case ALLOW_LVM_STAKE: {
      //     if (!forkController.pass(ForkBlockVersionEnum.VERSION_4_1)) {
      //     throw new ContractValidateException(
      //         "Bad chain parameter id [ALLOW_LVM_STAKE]");
      //   }
      //   if (value != 1 && value != 0) {
      //     throw new ContractValidateException(
      //         "This value[ALLOW_LVM_STAKE] is only allowed to be 1 or 0");
      //   }
      //   break;
      // }
      // case ALLOW_LVM_ASSET_ISSUE: {
      //   if (!forkController.pass(ForkBlockVersionEnum.VERSION_4_1)) {
      //     throw new ContractValidateException(
      //         "Bad chain parameter id [ALLOW_LVM_ASSET_ISSUE]");
      //   }
      //   if (value != 1 && value != 0) {
      //     throw new ContractValidateException(
      //         "This value[ALLOW_LVM_ASSET_ISSUE] is only allowed to be 1 or 0");
      //   }
      //   break;
      // }
  ...
  public enum ProposalType {         // current value, value range
    ...
    ACCOUNT_UPGRADE_COST(1), // 9999 USDL, [0, 100000000000] USDL
    CREATE_ACCOUNT_FEE(2), // 0.1 USDL, [0, 100000000000] USDL
    TRANSACTION_FEE(3), // 10 Sun/Byte, [0, 100000000000] USDL
    ASSET_ISSUE_FEE(4), // 1024 USDL, [0, 100000000000] USDL
    WITNESS_PAY_PER_BLOCK(5), // 16 USDL, [0, 100000000000] USDL
    WITNESS_STANDBY_ALLOWANCE(6), // 115200 USDL, [0, 100000000000] USDL
    CREATE_NEW_ACCOUNT_FEE_IN_SYSTEM_CONTRACT(7), // 0 USDL, [0, 100000000000] USDL
    ...
    KANDY_FEE(11), // 10 Sun, [0, 100000000000] USDL
    EXCHANGE_CREATE_FEE(12), // 1024 USDL, [0, 100000000000] USDL
    ...
    TOTAL_KANDY_LIMIT(17), // 50,000,000,000, [0, 100000000000000000]
    ALLOW_LVM_TRANSFER_TRC10(18), // 1, {0, 1}
    TOTAL_CURRENT_KANDY_LIMIT(19), // 50,000,000,000, [0, 100000000000000000]
    ALLOW_MULTI_SIGN(20), // 1, {0, 1}
    ALLOW_ADAPTIVE_KANDY(21), // 1, {0, 1}
    UPDATE_ACCOUNT_PERMISSION_FEE(22), // 100 USDL, [0, 100000] USDL
    MULTI_SIGN_FEE(23), // 1 USDL, [0, 100000] USDL
    ALLOW_PROTO_FILTER_NUM(24), // 0, {0, 1}
    ALLOW_ACCOUNT_STATE_ROOT(25), // 1, {0, 1}
    ALLOW_LVM_CONSTANTINOPLE(26), // 1, {0, 1}
    // ALLOW_SHIELDED_TRANSACTION(27), // 0, {0, 1}
    // SHIELDED_TRANSACTION_FEE(28), // 10 USDL, [0, 10000] USDL
    ADAPTIVE_RESOURCE_LIMIT_MULTIPLIER(29), // 1000, [1, 10000]
    ALLOW_CHANGE_DELEGATION(30), // 1, {0, 1}
    WITNESS_127_PAY_PER_BLOCK(31), // 160 USDL, [0, 100000000000] USDL
    ALLOW_LVM_SOLIDITY_059(32), // 1, {0, 1}
    ADAPTIVE_RESOURCE_LIMIT_TARGET_RATIO(33), // 10, [1, 1000]
    // SHIELDED_TRANSACTION_CREATE_ACCOUNT_FEE(34), // 1 USDL, [0, 10000] USDL
    FORBID_TRANSFER_TO_CONTRACT(35), // 1, {0, 1}
    ALLOW_SHIELDED_TRC20_TRANSACTION(39), // 1, 39
    ALLOW_PBFT(40),// 1,40
    ALLOW_LVM_ISTANBUL(41),//1, {0,1}
    // ALLOW_LVM_ASSET_ISSUE(42), // 0, 1
    // ALLOW_LVM_STAKE(43), // 0, 1

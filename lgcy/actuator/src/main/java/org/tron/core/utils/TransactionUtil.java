...
public class TransactionUtil {
  ...
  public static TransactionCapsule getTransactionSign(TransactionSign transactionSign) {
    ...
    TransactionCapsule usdl = new TransactionCapsule(transactionSign.getTransaction());
    usdl.sign(privateKey);
    return usdl;
  }
  ...
  public TransactionCapsule addSign(TransactionSign transactionSign)
    ...
    byte[] privateKey = transactionSign.getPrivateKey().toByteArray();
    TransactionCapsule usdl = new TransactionCapsule(transactionSign.getTransaction());
    usdl.addSign(privateKey, chainBaseManager.getAccountStore());
    return usdl;
  }
  ...
  public TransactionSignWeight getTransactionSignWeight(Transaction usdl) {
    ...
    try {
      ...
      if (usdl.getSignatureCount() > 0) {
        ...
        long currentWeight = TransactionCapsule.checkWeight(permission, usdl.getSignatureList(),
            Sha256Hash.hash(CommonParameter.getInstance()
                .isECKeyCryptoEngine(), usdl.getRawData().toByteArray()), approveList);

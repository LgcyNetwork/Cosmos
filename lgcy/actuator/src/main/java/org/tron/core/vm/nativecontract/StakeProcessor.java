...
public class StakeProcessor {
  ...
  public void process(StakeParam stakeParam, Repository repository)
    ...
    long voteCount = stakeParam.getStakeAmount() / ChainConstant.USDL_PRECISION;
  ...
  private void validateFreeze(byte[] ownerAddress, long frozenDuration,
    ...
    if (frozenBalance < ChainConstant.USDL_PRECISION) {
  ...
  private void validateVote(byte[] ownerAddress, Protocol.Vote vote, Repository repository)
    ...
    try {
      ...
      // usdl -> drop. The vote count is based on USDL
      sum = LongMath.checkedMultiply(sum, ChainConstant.USDL_PRECISION);
      if (sum > tronPower) {
        throw new ContractValidateException(
  ...
  private void executeFreeze(byte[] ownerAddress, long frozenDuration,
    ...
    repository
            .addTotalNetWeight(frozenBalance / ChainConstant.USDL_PRECISION);
  }

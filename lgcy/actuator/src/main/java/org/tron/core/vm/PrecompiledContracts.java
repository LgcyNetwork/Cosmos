...
public class PrecompiledContracts {
  ...
  public static PrecompiledContract getContractForAddress(DataWord address) {
    ...
    if (VMConfig.allowLvmSolidity059() && address.equals(batchValidateSignAddr)) {
      return batchValidateSign;
    }
    if (VMConfig.allowLvmSolidity059() && address.equals(validateMultiSignAddr)) {
  ...
  public static abstract class PrecompiledContract {
    ...
    public abstract long getKandyForData(byte[] data);
  ...
  public static class Identity extends PrecompiledContract {
    ...
    @Override
    public long getKandyForData(byte[] data) {

      // kandy charge for the execution:
  ...
  public static class ModExp extends PrecompiledContract {
    ...
    public long getKandyForData(byte[] data) {
      ...
      BigInteger kandy = BigInteger.valueOf(multComplexity)
          .multiply(BigInteger.valueOf(Math.max(adjExpLen, 1)))
          .divide(GQUAD_DIVISOR);

      return isLessThan(kandy, BigInteger.valueOf(Long.MAX_VALUE)) ? kandy.longValueExact()
          : Long.MAX_VALUE;
    }
  ...
  public static class BN128Addition extends PrecompiledContract {
    ...
    public long getKandyForData(byte[] data) {
      if (VMConfig.allowLvmIstanbul()) {
        return getKandyForDataIstanbul(data);
      }
      return 500;
    }

    private long getKandyForDataIstanbul(byte[] data) {
      return 150;
    }

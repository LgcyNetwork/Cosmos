...
public class TransferActuator extends AbstractActuator {
  ...
  public boolean validate() throws ContractValidateException {
    ...
    if (Arrays.equals(toAddress, ownerAddress)) {
      throw new ContractValidateException("Cannot transfer USDL to yourself.");
    }

    AccountCapsule ownerAccount = accountStore.get(ownerAddress);

    if (ownerAccount == null) {
      throw new ContractValidateException("Validate TransferContract error, no OwnerAccount11.");
    }
    ...
    try {
      ...
      //after ForbidTransferToContract proposal, send usdl to smartContract by actuator is not allowed.
      if (dynamicStore.getForbidTransferToContract() == 1
          && toAccount != null
          && toAccount.getType() == AccountType.Contract) {

        throw new ContractValidateException("Cannot transfer USDL to a smartContract.");

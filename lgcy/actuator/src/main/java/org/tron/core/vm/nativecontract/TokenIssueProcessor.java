...
public class TokenIssueProcessor {
    ...
    public void execute(Object contract, Repository repository) {
        ...
        // spend 1024usdl for assetissue, send to blackhole address
    ...
    public void validate(Object contract, Repository repository) throws ContractValidateException {
        ...
        if ((USDL.equalsIgnoreCase(ByteArray.toStr(tokenIssueParam.getName())))) {
            throw new ContractValidateException("assetName can't be usdl or null");
        }
        if (tokenIssueParam.getPrecision() < 0 || tokenIssueParam.getPrecision() > TOKEN_ISSUE_PRECISION) {
            throw new ContractValidateException("precision cannot exceed 18");
        }

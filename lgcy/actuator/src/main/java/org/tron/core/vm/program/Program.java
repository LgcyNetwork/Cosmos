...
public class Program {
  ...
  private static final String REFUND_KANDY_FROM_MESSAGE_CALL = "refund kandy from message call";
  ...
  /**
   * @param transferAddress the address send USDL to.
   * @param value the USDL value transferred in the internal transaction
   */
  private InternalTransaction addInternalTx(DataWord kandyLimit, byte[] senderAddress,
      ...
      try {
        ...
        if (VMConfig.allowLvmConstantinople()) {
          throw new TransferException(
              "transfer all token or transfer all usdl failed in suicide: %s", e.getMessage());
        }
  ...
  private void suicideFreezeBalanceAndVote(byte[] owner, byte[] obtainer, Repository repository) {
    ...
    if (FastByteComparisons.compareTo(obtainer, 0, 20,
      ...
      repository
              .addTotalNetWeight(-unfreezeBalance / Parameter.ChainConstant.USDL_PRECISION);
    }
    //vote
    {
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
    ...
    if (VMConfig.allowLvmConstantinople()) {
      ...
      if (!contractAlreadyExists) {
        ...
        if (isCreate2) {
          builder.setUsdlHash(ByteString.copyFrom(rootTransactionId));
        }
    ...
    // actual kandy subtract
    DataWord kandyLimit = this.getCreateKandy(getKandyLimitLeft());
    spendKandy(kandyLimit.longValue(), "internal call");
    ...
    ProgramInvoke programInvoke = programInvokeFactory.createProgramInvoke(
        ...
        getVmShouldEndInUs(), kandyLimit.longValueSafe());
    if (isConstantCall()) {
    ...
    long afterSpend =
        programInvoke.getKandyLimit() - createResult.getKandyUsed() - saveCodeKandy;
    if (!createResult.isRevert()) {
      if (afterSpend < 0) {
        createResult.setException(
            Exception.notEnoughSpendKandy("No kandy to save just created contract code",
                saveCodeKandy, programInvoke.getKandyLimit() - createResult.getKandyUsed()));
      } else {
        createResult.spendKandy(saveCodeKandy);
    ...
    // 5. REFUND THE REMAIN Kandy
    refundKandyAfterVM(kandyLimit, createResult);
  }

  public void refundKandyAfterVM(DataWord kandyLimit, ProgramResult result) {

    long refundKandy = kandyLimit.longValueSafe() - result.getKandyUsed();
    if (refundKandy > 0) {
      refundKandy(refundKandy, "remain kandy from the internal call");
      if (logger.isDebugEnabled()) {
        logger.debug("The remaining kandy is refunded, account: [{}], kandy: [{}] ",
            Hex.toHexString(
  ...
  public void callToAddress(MessageCall msg) {
    ...
    if (getCallDeep() == MAX_DEPTH) {
      stackPushZero();
      refundKandy(msg.getKandy().longValue(), " call deep limit reach");
      return;
    }
    ...
    try {
      ...
      if (VMConfig.allowLvmConstantinople()) {
        refundKandy(msg.getKandy().longValue(), "endowment out of long range");
        throw new TransferException("endowment out of long range");
      } else {
        throw e;
      }
    }
    // transfer USDL validation
    byte[] tokenId = null;
    ...
    if (!isTokenTransfer) {
      ...
      if (senderBalance < endowment) {
        stackPushZero();
        refundKandy(msg.getKandy().longValue(), REFUND_KANDY_FROM_MESSAGE_CALL);
        return;
      }
    } else {
    ...
    if (byTestingSuite()) {
      ...
      if (!isTokenTransfer) {
        try {
          ...
          if (VMConfig.allowLvmConstantinople()) {
            refundKandy(msg.getKandy().longValue(), REFUND_KANDY_FROM_MESSAGE_CALL);
            throw new TransferException("transfer usdl failed: %s", e.getMessage());
          }
          throw new BytecodeExecutionException(VALIDATE_FOR_SMART_CONTRACT_FAILURE, e.getMessage());
        }
        deposit.addBalance(senderAddress, -endowment);
    ...
    if (isNotEmpty(programCode)) {
      ...
      ProgramInvoke programInvoke = programInvokeFactory.createProgramInvoke(
          ...
          byTestingSuite(), vmStartInUs, getVmShouldEndInUs(), msg.getKandy().longValueSafe());
      if (isConstantCall()) {
        programInvoke.setConstantCall();
      }
      VM vm = new VM(config);
    ...
    // 5. REFUND THE REMAIN USDL_POWER
    if (callResult != null) {
      BigInteger refundKandy = msg.getKandy().value()
          .subtract(BIUtil.toBI(callResult.getKandyUsed()));
      if (BIUtil.isPositive(refundKandy)) {
        refundKandy(refundKandy.longValueExact(), "remaining kandy from the internal call");
        if (logger.isDebugEnabled()) {
          logger.debug("The remaining kandy refunded, account: [{}], kandy: [{}] ",
              Hex.toHexString(senderAddress),
              refundKandy.toString());
        }
      }
    } else {
      refundKandy(msg.getKandy().longValue(), "remaining kandy from the internal call");
    }
  }
  ...
  public void spendKandy(long kandyValue, String opName) {
    if (getKandylimitLeftLong() < kandyValue) {
      throw new OutOfKandyException(
          "Not enough kandy for '%s' operation executing: curInvokeKandyLimit[%d],"
              + " curOpKandy[%d], usedKandy[%d]",
          opName, invoke.getKandyLimit(), kandyValue, getResult().getKandyUsed());
    }
    getResult().spendKandy(kandyValue);
  }
  ...
  public void checkCPUTimeLimit(String opName) {
    ...
    if (vmNowInUs > getVmShouldEndInUs()) {
      logger.info(
          ...
          getVmShouldEndInUs(), vmNowInUs, gelvmStartInUs());
      throw Exception.notEnoughTime(opName);
    }

  }

  public void spendAllKandy() {
    spendKandy(getKandyLimitLeft().longValue(), "Spending all remaining");
  }

  public void refundKandy(long kandyValue, String cause) {
    logger
        .debug("[{}] Refund for cause: [{}], kandy: [{}]", invoke.hashCode(), cause, kandyValue);
    getResult().refundKandy(kandyValue);
  }

  public void futureRefundKandy(long kandyValue) {
    logger.debug("Future refund added: [{}]", kandyValue);
    getResult().addFutureRefund(kandyValue);
  }
  ...
  public long getKandylimitLeftLong() {
    return invoke.getKandyLimit() - getResult().getKandyUsed();
  }

  public DataWord getKandyLimitLeft() {
    return new DataWord(invoke.getKandyLimit() - getResult().getKandyUsed());
  }
  ...
  public void fullTrace() {
    if (logger.isTraceEnabled() || listener != null) {
      ...
      logger.trace("\n  Spent Drop: [{}]/[{}]\n  Left Kandy:  [{}]\n",
          getResult().getKandyUsed(),
          invoke.getKandyLimit(),
          getKandyLimitLeft().longValue());
      ...
      globalOutput.append("\n\n  Spent Kandy: ").append(getResult().getKandyUsed());
  ...
  public void saveOpTrace() {
    if (this.pc < ops.length) {
      trace.addOp(ops[pc], pc, getCallDeep(), getKandyLimitLeft(), traceListener.resetActions());
    }
  }
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (getCallDeep() == MAX_DEPTH) {
      stackPushZero();
      this.refundKandy(msg.getKandy().longValue(), " call deep limit reach");
      return;
    }
    ...
    long requiredKandy = contract.getKandyForData(data);
    if (requiredKandy > msg.getKandy().longValue()) {
      // Not need to throw an exception, method caller needn't know that
      // regard as consumed the kandy
      this.refundKandy(0, CALL_PRE_COMPILED); //matches cpp logic
      this.stackPushZero();
    } else {
      ...
      if (out.getLeft()) { // success
        this.refundKandy(msg.getKandy().longValue() - requiredKandy, CALL_PRE_COMPILED);
        this.stackPushOne();
        returnDataBuffer = out.getRight();
        deposit.commit();
      } else {
        // spend all kandy on failure, push zero and revert state changes
        this.refundKandy(0, CALL_PRE_COMPILED);
        this.stackPushZero();
  ...
  public DataWord getCallKandy(OpCode op, DataWord requestedKandy, DataWord availableKandy) {
    return requestedKandy.compareTo(availableKandy) > 0 ? availableKandy : requestedKandy;
  }

  public DataWord getCreateKandy(DataWord availableKandy) {
    return availableKandy;
  }
  ...
  public long gelvmStartInUs() {
    return this.invoke.gelvmStartInUs();
  }
  ...
  private void createAccountIfNotExist(Repository deposit, byte[] contextAddress) {
    if (VMConfig.allowLvmSolidity059()) {
      //after solidity059 proposal , allow contract transfer trc10 or USDL to non-exist address(would create one)
  ...
  @SuppressWarnings("serial")
  public static class OutOfKandyException extends BytecodeExecutionException {

    public OutOfKandyException(String message, Object... args) {
      super(format(message, args));
    }
  }
  ...
  public static class Exception {
    ...
    public static OutOfKandyException notEnoughOpKandy(OpCode op, long opKandy,
        long programKandy) {
      return new OutOfKandyException(
          "Not enough kandy for '%s' operation executing: opKandy[%d], programKandy[%d];", op,
          opKandy,
          programKandy);
    }

    public static OutOfKandyException notEnoughOpKandy(OpCode op, DataWord opKandy,
        DataWord programKandy) {
      return notEnoughOpKandy(op, opKandy.longValue(), programKandy.longValue());
    }

    public static OutOfKandyException notEnoughSpendKandy(String hint, long needKandy,
        long leftKandy) {
      return new OutOfKandyException(
          "Not enough kandy for '%s' executing: needKandy[%d], leftKandy[%d];", hint, needKandy,
          leftKandy);
    }
    ...
    public static OutOfKandyException kandyOverflow(BigInteger actualKandy,
        BigInteger kandyLimit) {
      return new OutOfKandyException("Kandy value overflow: actualKandy[%d], kandyLimit[%d];",
          actualKandy.longValueExact(), kandyLimit.longValueExact());
    }

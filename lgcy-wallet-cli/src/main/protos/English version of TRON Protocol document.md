...
```
     ...
     `asset`: other assets expect USDL in this account – e.g. _{<“WishToken”,66666>,<”Dogie”,233>}_.
...
```    
     ...
     `amount`: the amount of USDL – e.g. _12534_.  
...
```
     ...
     `usdl_num`: the number of TRONIX – e.g._232241_.  
...
+	Inventory is mainly used to inform peer nodes the list of items.  
    ...
    `type`: what type this `Inventory` is. – e.g. _0_ stands for `USDL`.  
    `ids`: ID of things in this `Inventory`.

    Two `Inventory` types: `USDL` and `BLOCK`.  
    `USDL`: transaction.  
    `BLOCK`: block.
    ...
    `type`: type of items – e.g. _1_ stands for `USDL`.  
    ...
    `Items` have four types: `ERR`, `USDL`, `BLOCK` and `BLOCKHEADER`.  
    `ERR`: error.  
    `USDL`: transaction.  
    `BLOCK`: block.  

...
public class WalletApi {
  ...
  public static UpdateKandyLimitContract createUpdateKandyLimitContract(
      byte[] owner, byte[] contractAddress, long originKandyLimit) {

    UpdateKandyLimitContract.Builder builder = UpdateKandyLimitContract.newBuilder();
    builder.setOwnerAddress(ByteString.copyFrom(owner));
    builder.setContractAddress(ByteString.copyFrom(contractAddress));
    builder.setOriginKandyLimit(originKandyLimit);
    return builder.build();
  }
  ...
  public static CreateSmartContract createContractDeployContract(
    ...
    builder
        ...
        .setOriginKandyLimit(originKandyLimit);
  ...
  public byte[] generateContractAddress(byte[] ownerAddress, Transaction usdl) {
    // get tx hash
    byte[] txRawDataHash = Sha256Sm3Hash.of(usdl.getRawData().toByteArray()).getBytes();
  ...
  public boolean updateSetting(byte[] owner, byte[] contractAddress,
    ...
    if (transactionExtention == null || !transactionExtention.getResult().getResult()) {
      System.out.println("RPC create usdl failed!");
      if (transactionExtention != null) {
  ...
  public boolean updateKandyLimit(byte[] owner, byte[] contractAddress, long originKandyLimit)
      throws IOException, CipherException, CancelException {
    if (owner == null) {
      owner = getAddress();
    }

    UpdateKandyLimitContract updateKandyLimitContract = createUpdateKandyLimitContract(owner,
        contractAddress, originKandyLimit);

    TransactionExtention transactionExtention = rpcCli.updateKandyLimit(updateKandyLimitContract);
    if (transactionExtention == null || !transactionExtention.getResult().getResult()) {
      System.out.println("RPC create usdl failed!");
      if (transactionExtention != null) {
        System.out.println("Code = " + transactionExtention.getResult().getCode());
  ...
  public boolean triggerContract(
    ...
    if (transactionExtention == null || !transactionExtention.getResult().getResult()) {
      System.out.println("RPC create call usdl failed!");
      System.out.println("Code = " + transactionExtention.getResult().getCode());
      System.out
          .println("Message = " + transactionExtention.getResult().getMessage().toStringUtf8());
      return false;
  ...
  public static boolean sendShieldedCoinWithoutAsk(PrivateParametersWithoutAsk privateParameters,
    ...
    BytesMessage usdlHash = rpcCli.getShieldedTransactionHash(transactionExtention.getTransaction());
    if (usdlHash == null || usdlHash.getValue().toByteArray().length != 32) {
    ...
    for (int i = 0; i < spendDescList.size(); i++) {
      ...
      builder.setTxHash(ByteString.copyFrom(usdlHash.getValue().toByteArray()));
